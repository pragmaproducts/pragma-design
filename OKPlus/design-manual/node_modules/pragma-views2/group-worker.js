class GroupWorker {
    ...
    filterRecords(args) {
        ...
        if (cachedItems != null) {
            if ((args.filterString || "").length === 0) {
                ...
                results = cachedItems.data.filter(this.fuzzySearch(filterString));
                for(const item of cachedItems.data) {
                    results = results.concat(this.filterNestedCollections(item, filterString));
                }
            }
        }
    ...
    filterNestedCollections(item, filterString) {
        let results = [];
        const collections = Object.values(item).filter(val => val.items != null);
        for(const collection of collections) {
            results = results.concat(collection.filter(this.fuzzySearch(filterString)));
        }
        return results;
    }

    fuzzySearch(filterString) {
        // Filter items using some function. Tested performance against for loop and similar results
        return record => Object.values(record).some(v => (v || "").toString().toLowerCase().indexOf(filterString) > -1);
    }
