...
import {contextItems} from "../context-menu/context-items.js";
import {getEventPosition} from "../../../lib/dom-helper.js";

export default class AdvancedTree extends ViewBase {

    _showContextMenu(event) {
        this.tree.scroller.style.pointerEvents = "none";
        const point = getEventPosition(event);
        const elements = document.elementsFromPoint(point.x, point.y);
        const treeItem = elements.find(el=> el.classList.contains("tree-item"));

        this.tree.scroller.style.pointerEvents = "auto";

        if(treeItem != null) {
            const dataItem = this.tree.dataStore.data[treeItem.__dataIndex];
            this.tree.dataStore.toggleSelection(treeItem.__dataIndex, true);
            const menuItems = cloneObject(contextItems);
            if(menuItems == null) return;
            const options = {
                callback: (menuItem)=> {
                    console.log(menuItem);
                },
                menuItems: cloneObject(menuItems),
                event: event
            };
    ...
    disconnectedCallback() {
        window.eventEmitter.emit("unregister-context-menu", { element: this });
    ...
    remoteDsCallback(def, payload, parent, routerParameters, queryOptions) {
        ...
        if (parent != null) {
            let array =  [];
            switch (parent.model.type) {
                case "folder":
                    array = this.data.assets;
                    break;
                case "asset":
                    array = this.data.assetFolders;
                    break;
                case "assetFolder":
                    array = this.data.componentsAndTasks.filter((item)=> item.type === parent.model.subType);
                    break;


            }
            return array.filter((item)=> item.parentId == parent.model.id);
        }
        
        return cloneObject(this.data.folders);
    }
    ...
    schemaLoaded(event) {
        ...
        this.tree = event.target.querySelector("pragma-advanced-tree");
        this.contextMenuEventHandler = this._showContextMenu.bind(this);
        const cmoptions = {
            element: this.tree,
            eventHandler: this.contextMenuEventHandler
        };
        window.eventEmitter.emit("register-context-menu", cmoptions);
    }
